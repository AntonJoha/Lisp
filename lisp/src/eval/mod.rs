...
pub fn call_func(fun: lexer::Entry, arguments: VecDeque<Value>) -> Value {
    match fun.t.clone(){
        ...
        _ => {
            operator::operator_eval(fun, arguments)
        }
    }
}
...
pub fn process(input: &mut VecDeque<lexer::Entry>) -> Value {
    ...
    let fun = match input.pop_front() {
        Some(e) => e,
        _ => lexer::Entry{ lexeme : "Error".to_string(), t: lexer::Token::Error},
    };
        ...
        l => {
            ...
            loop {
                let t: lexer::Entry = match input.pop_front() {
                    Some(e) => e,
                    _ => {lexer::Entry { lexeme: ")".to_string(), t: lexer::Token::Close}}
                };
                match t.t.clone() {
                    lexer::Token::Close => {
    ...
    if to_return.len() > 1 {
        Value { literal: "".to_string(), t: lexer::Token::Open, list: to_return}
    }
    else if to_return.len() == 1 {
        match to_return.pop_front() {
            Some(v) => {
                Value { literal: v.literal, t: v.t, list: v.list}
            },
            _ => {
                Value { literal: "Error".to_string(), t: lexer::Token::Error, list: VecDeque::new() }
            }
        }
    }
    else {
        Value { literal: "0".to_string(), t: lexer::Token::Number, list: VecDeque::new()}
    }
}
